...
describe('plan', () => {
  it('plan expected output', () => {
    const returnTestFramework = plan().build(complexPlan)('f1')
    return promise(returnTestFramework).then(
      (data) =>
      ...
      (error) =>
        assert.fail('Future was not expected to be rejected with: ' + JSON.stringify(error))
    )
  })
  ...
  it('plan expected output using mockups', () => {
    const returnTestFramework = plan({
  ...
  it('if a piped function return an instance of Error or subclass of Error, the final result will be a resolve with the error object', () => {
    ...
    return promise(returnTestFramework).then(
      (dataError) =>
        assert.deepStrictEqual(Object.entries(dataError.data), {
          code: '400',
          message: 'bankingToFetch is empty',
        }),
      () => {}
  ...
  it('Plan without Future and complex nesting', () => {
    const result = plan().build([[[(x) => x + 1], (x) => x + 1], [(x) => x + 1], [(x) => x + 1]])(1)

    assert.deepStrictEqual(result, [3, 2, 2])
  })
  ...
  it('Plan without Future and complex nesting', () => {
    ...
    assert.deepStrictEqual(result, [5, 5])
  })
  ...
  it('Plan without Future and complex nesting', () => {
    ...
    assert.deepStrictEqual(result, [[2, 2], 2, 2, 2])
  })
  ...
  function getCreditCardBalancesProm(data) {
    return Promise.resolve(
      data.creditCardsToFetch.map((card) => ({
        account: card,
        ...bankDB.balancesCreditCards[card],
      }))
    )
  }
  ...
  it('Complex plan with Promises', () => {
    return plan().build(complexPlanWithPromises)('f1').then(
      (data) =>
        assert.deepStrictEqual(data, {
          name: 'Jose Marin',
          holdings: [
      ...
      (error) =>
        assert.fail('Promise was not expected to be rejected with: ' + JSON.stringify(error))
    )
  })
})
